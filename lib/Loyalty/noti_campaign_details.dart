...
class NotiCampaignDetails extends StatefulWidget {
  final noti;
  NotiCampaignDetails(this.noti);

  @override
  _NotiCampaignDetailsState createState() => _NotiCampaignDetailsState();
}

class _NotiCampaignDetailsState extends State<NotiCampaignDetails> {
    ...
    if (response != null) {
      if (response.statusCode == 200) {
        ...
        if (result['returncode'] == "300") {
          _couponname = result['data'][0]['coupon'];
          if (result['data'][0]['campaigndetails'].length > 0) {
            _passData = result['data'][0]['campaigndetails'][0];
          ...
          _wonprices = result['data'][0]['wonprice'];
          _loading = false;
          setState(() {});
  ...
  Widget build(BuildContext context) {
                          ...
                          : GestureDetector(
                              onTap: () async {
                                if (!_loading) {
                                  var couponData = {
                                    "coupon": _couponname,
                                    "winnerid": _wonprices[i]['primarykey']
                                  };
                                  createQRcode(couponData);
                                  // displayLogout(i);
                                }
    ...
    return Scaffold(
      ...
      body: (!_loading && _passData.length > 0)
          ? ListView(
              children: [
                      ...
                      RichText(
                        text: TextSpan(
                          children: [
                            ...
                            TextSpan(
                              text:
                                  "  " + showDate(_passData['activationdate']),
                              style: TextStyle(
                                color: Colors.black54,
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
                      ...
                      RichText(
                        text: TextSpan(
                          children: [
                            ...
                            TextSpan(
                              text:
                                  "  " + showDate(_passData['expirationdate']),
                              style: TextStyle(
                                color: Colors.black54,
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
